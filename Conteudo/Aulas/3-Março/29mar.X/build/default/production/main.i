...
void escrever_num_frac(double numero,int precisao){
    ...
    while( aux<cont+precisao){
        aux++;
        numero*=10;
        digito = floorf(numero);
        numero-=digito;
...
void escrever_num_int(int numero){

    int esq, dir;

    if(numero < 10)
        escrever_char(48 + numero);
    else{
        esq = numero / 10;
        dir = numero % 10;

        escrever_char(48 + esq);
        escrever_char(48 + dir);
    }
}


void imprime_horario (int minutos, int segundos){

    int dminutos = floorf(minutos/10.0);
    int uminutos = minutos - dminutos*10;

    int dsegundos = floorf(segundos/10.0);
    int usegundos = segundos - dsegundos*10;
...
void __attribute__((picinterrupt(("high_priority")))) isr_inter(void){
...
void main(void) {
    ...
    ((void)(TMR0H=((3036)>>8),TMR0L=((3036)&0xFF)));
